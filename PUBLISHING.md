...
2. Tag a release: `git tag 0.1.1 && git push origin 0.1.1`.
...
> The library's Maven `group` is set to `com.github.ccBiver.chartkit` (matching JitPack's coordinate) and the git **tag must equal the version** (`0.1.1`, no `v` prefix). This keeps the compose → core transitive dependency (`com.github.ccBiver.chartkit:chartkit-core:0.1.1`) resolvable on JitPack, so consumers declare only the compose artifact.
